...
for folder in folder_list:
    ...
    time.sleep(1)
    pyautogui.click(400, 76)
    time.sleep(1)
    # # 키보드의 Shift와 Down 방향 키를 동시에 20초간 누르기
    # pyautogui.keyDown('shift')  # Shift 키를 누른 상태로 유지
    # pyautogui.keyDown('down')   # Down 방향 키를 누른 상태로 유지
    # time.sleep(20)              # 20초간 기다리기
    # pyautogui.keyUp('down')     # Down 방향 키를 놓기
    # pyautogui.keyUp('shift')    # Shift 키를 놓기
    ...
    for _ in tqdm(range(file_count), desc=f"Processing files in {folder}"):
        ...
        # Serial 값 복사 및 저장
        pyautogui.click(460, 689)
        pyautogui.dragTo(310, 689, duration=1)
