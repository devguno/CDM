...
root_directory = 'Z:/child_tmt/Child TMT Device #1'
# 결과를 저장할 기본 경로
...
for subdir, _, files in tqdm(os.walk(root_directory), desc="Processing folders"):
    ...
    for filename in tqdm([f for f in files if f.endswith('.xml')], desc=f"Processing XML files in {os.path.basename(subdir)}"):
