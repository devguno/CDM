...
def convert_pdf_to_dict(paths, output_path):
    ...
    for path in tqdm(paths):
        for page in range(1,5):
            ...
            # 파일명 추출 및 Pickle 파일로 저장
            filename = os.path.splitext(os.path.basename(path))[0]  # PDF 파일의 파일명 추출 (확장자 제외)
            save_pickle(total_dict, f"{output_path}/{filename}.pickle")
            break
...
if __name__ == '__main__':
    root_dir = '/workspace/gunoroh/sftp/Holter_raw_sig'
