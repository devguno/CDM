...
def normalize_timestamp(timestamp_str, base_date):
    """
    예: '08:20:11 21-Apr' + '2007-04-20' → '2007-04-21 08:20:11'
    """
    try:
        time_part, date_part = timestamp_str.strip().split(" ", 1)
        full_str = f"{date_part}-{base_date[:4]} {time_part}"
...
def parse_heart_rates_section(text, base_date):
    heart_rates_data = {}
    patterns = [
    ...
    for pattern, main_tag, sub_tag in patterns:
        ...
        if match:
            if sub_tag:
                value = match.group(2)
                # Timestamp인 경우만 날짜 변환 적용
                if sub_tag == 'Timestamp':
                    value = normalize_timestamp(value, base_date)
                heart_rates_data[main_tag] = {'Value': match.group(1), sub_tag: value}
            else:
                heart_rates_data[main_tag] = {'Value': match.group(1)}
        else:
            if sub_tag:
                heart_rates_data[main_tag] = {'Value': "Unknown", sub_tag: "Unknown"}
            else:
                heart_rates_data[main_tag] = {'Value': "Unknown"}
...
def process_pdf_files(file_dir, json_dir):
    ...
    for pdf_path in tqdm(pdf_files, desc="Processing PDF Files"):
        try:
            ...
            patient_info['HookupDate'] = parse_date(patient_info['HookupDate'])
            general_data = parse_general_section(extracted_text)
            heart_rates_data = parse_heart_rates_section(extracted_text, formatted_hookup_date)
            ...
            def parse_and_format_section(section_text, patterns, base_date):
                section_data = {}
                for pattern, tags in patterns:
                    matches = extract_grouped_matches(pattern, section_text, len(tags))
                    for i, tag in enumerate(tags):
                        val = matches[i]
                        if 'Timestamp' in tag and val != "Unknown":
                            val = normalize_timestamp(val, base_date)
                        section_data[tag] = val
                return section_data
 
            ventriculars_data = parse_and_format_section(ventriculars_section, ventriculars_patterns, formatted_hookup_date)
            supraventriculars_data = parse_and_format_section(supraventriculars_section, supraventriculars_patterns, formatted_hookup_date)
...
def main():
    base_dirs = r'/home/coder/workspace/sftp/ann_0314'  
    json_dir = r'/home/coder/workspace/sftp/raw_json2'  
